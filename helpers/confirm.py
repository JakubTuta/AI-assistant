...
The web UI has always confirmed these before running them, and still does — a
click there passes the dialog and runs the job directly. The voice path never
did, and voice is the higher-risk channel, because "delete the email from Anna"
and "delete the email from Hannah" are one misrecognition apart. This gate sits
in the agent loop, so it covers voice and typed chat alike.
